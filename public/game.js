...
class MainGame extends Singleton{start(){let config=new GameConfig({parent:"game",scene:[StartScene,GameScene]});BaseGame.instance.start(config);let em=new EventManager();em.setStart(game);}onMessage(data){console.log(data);}}
...
class App extends react.exports.Component{constructor(){super();this.state={started:false};this.chatbox=/*#__PURE__*/React.createRef();}render(){return/*#__PURE__*/React.createElement("div",{className:"main"},/*#__PURE__*/React.createElement("div",{className:`main-first ${this.state.started?"hidden":""}`},/*#__PURE__*/React.createElement("button",{id:"startButton",onClick:()=>{this.startGame.bind(this)();}},"Start")),/*#__PURE__*/React.createElement("div",{className:`main-second ${this.state.started?"":"hidden"}`},/*#__PURE__*/React.createElement(ChatBox,{ref:this.chatbox})));}startGame(){this.setState({started:true});let game=new MainGame();MainGame.instance.onMessage=data=>{this.chatbox.current.chat(data);};game.start();}}
