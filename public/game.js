...
{key:"fire_sword_idle",route:"weapons/fire_sword",frames:["fire_sword1","fire_sword2","fire_sword3","fire_sword4","fire_sword5","fire_sword6","fire_sword7","fire_sword8","fire_sword9","fire_sword10"],origin:{x:0.5,y:0.5}}];}}
...
class Player extends Entity{constructor(id,x,y,width=32,height=32){super(x,y,width,height);this.id=id;this.velX=0;this.velY=0;}setSprite(sprite){this.sprite=sprite;this.sprite.x=this.x;this.sprite.y=this.y;this.sprite.width=this.width;this.sprite.height=this.height;}setName(name){this.name=name;this.name.x=this.x-this.width*.6;this.name.y=this.y-this.height;}setWeapon(weapon){this.weapon=weapon;this.weapon.x=this.x;this.weapon.y=this.y;this.weapon.sprite.x=this.weapon.x;this.weapon.sprite.y=this.weapon.y;this.weapon.sprite.setOrigin(this.weapon.origin.x,this.weapon.origin.y);}hideWeapon(){this.weapon.sprite.visible=false;}showWeapon(){this.weapon.sprite.visible=true;}canMove(){if(!BaseGame.instance.checkCollides(this))return true;return false;}move(x,y,direction="right"){this.sprite.x=x;this.sprite.y=y;this.name.x=x-this.width*.6;this.name.y=y-this.height;if(this.weapon){this.weapon.x=x;this.weapon.y=y;this.weapon.sprite.x=this.weapon.x;this.weapon.sprite.y=this.weapon.y;this.weapon.sprite.setOrigin(this.weapon.origin.x,this.weapon.origin.y);}if(direction=="left")this.animate("walking",-1);else if(direction=="right")this.animate("walking",1);else if(direction=="up"){if(this.currentAnim!="up"){this.animate("jump",1,80,{x:0.75,y:0.75},false);}else {this.animate("jump_high",1,80,{x:0.75,y:0.75},false);}}else if(direction=="down"){if(this.currentAnim!="down"){this.animate("fall",1,80,{x:0.75,y:0.75},false);}else {this.animate("fall_low",1,80,{x:0.75,y:0.75},false);}}else {this.animate("idle",1,10,{x:0.5,y:0.5},true);}this.currentAnim=direction;}animate(animation,direction,speed=10,origin,loop=true){this.sprite.anims.playAfterDelay(animation,1);this.sprite.setScale(direction,1);if(origin)this.sprite.setOrigin(origin.x,origin.y);if(this.weapon)this.weapon.sprite.anims.playAfterDelay(`${this.weapon.animation}_${animation}`,1);}destroy(){this.sprite.destroy();this.name.destroy();if(this.weapon)this.weapon.sprite.destroy();}}

class Network extends Singleton{constructor(){super();}connect(url,game,instanced,callback){Network.instance.game=game;let socket=new WebSocket(url);Network.instance.socket=socket;socket.onopen=()=>{console.log("Connected to server");callback();};socket.onmessage=event=>{let{type,data}=JSON.parse(event.data);let player;let sprite;let uText;let weapon;switch(type){case"login":if(data.success){console.log("Logged in");}else {console.log("Login failed");}break;case"selfJoin":console.log("Self joined",data.name);player=new Player(data.name,data.x,data.y);window.globalPlayerEntity=player;sprite=Network.instance.game.add.sprite(data.x,data.y,"idle1");weapon={sprite:Network.instance.game.add.sprite(data.x,data.y,"fire_sword1"),animation:"fire_sword",origin:{x:0.5,y:0.5}};uText=Network.instance.game.add.text(data.x,data.y,data.name,{font:"16px Arial",fill:"#ffffff",align:"center"});player.setSprite(sprite);player.setName(uText);player.setWeapon(weapon);player.animate("idle");instanced.addEntity(player);BaseGame.instance.camera.startFollow(player.sprite);break;case"playerJoin":if(data.name!==Network.instance.username){console.log("Player joined",data.name);player=new Player(data.name,data.x,data.y);sprite=Network.instance.game.add.sprite(data.x,data.y,"idle1");weapon={sprite:Network.instance.game.add.sprite(data.x,data.y,"fire_sword1"),animation:"fire_sword",origin:{x:0.5,y:0.5}};uText=Network.instance.game.add.text(data.x,data.y,data.name,{font:"16px Arial",fill:"#ffffff",align:"center"});player.setSprite(sprite);player.setName(uText);player.setWeapon(weapon);player.animate("idle");instanced.addEntity(player);}break;case"move":player=BaseGame.instance.findObject(data.name);if(player)player.move(data.x,data.y,data.direction);break;case"loadMap":instanced.loadMap(data.map);break;case"playerLeave":player=BaseGame.instance.findObject(data.name);if(player)player.destroy();break;case"setChange":BaseGame.instance.updates.set(data.x,data.y,data);break;case"deleteBlock":instanced.deleteBlock(data);break;case"chat":MainGame.instance.onMessage(data);break;case"worldMenu":MainGame.instance.onWorldMenu(data);break;default:console.log(`Unknown message type: ${type}`);break;}};socket.onclose=()=>{console.log("Disconnected from server");};}setActive(game){Network.instance.game=game;}send(data){this.socket.send(JSON.stringify(data));}}
