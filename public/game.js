...
class InputSystem extends Singleton{linkEvent(event,callback){this.game.input.keyboard.on(event,callback);}}
...
{key:"fire_sword",route:"weapons/fire_sword",frames:["fire_sword1","fire_sword2","fire_sword3","fire_sword4","fire_sword5","fire_sword6","fire_sword7","fire_sword8","fire_sword9","fire_sword10"],origin:{x:0,y:0}}];}}
...
class Player extends Entity{constructor(id,x,y,width=32,height=32){super(x,y,width,height);this.id=id;this.velX=0;this.velY=0;}setSprite(sprite){this.sprite=sprite;this.sprite.x=this.x;this.sprite.y=this.y;this.sprite.width=this.width;this.sprite.height=this.height;}setName(name){this.name=name;this.name.x=this.x-this.width*.6;this.name.y=this.y-this.height;}canMove(){if(!BaseGame.instance.checkCollides(this))return true;return false;}move(x,y,direction="right"){this.sprite.x=x;this.sprite.y=y;this.name.x=x-this.width*.6;this.name.y=y-this.height;if(direction=="left")this.animate("walking",-1);else if(direction=="right")this.animate("walking",1);else if(direction=="up"){if(this.currentAnim!="up"){this.animate("jump",1,80,{x:0.75,y:0.75},false);}else {this.animate("jump_high",1,80,{x:0.75,y:0.75},false);}}else if(direction=="down"){if(this.currentAnim!="down"){this.animate("fall",1,80,{x:0.75,y:0.75},false);}else {this.animate("fall_low",1,80,{x:0.75,y:0.75},false);}}else {this.animate("idle",1,10,{x:0.5,y:0.5},true);}this.currentAnim=direction;}animate(animation,direction,speed=10,origin,loop=true){this.sprite.play(animation,speed,loop);this.sprite.setScale(direction,1);if(origin)this.sprite.setOrigin(origin.x,origin.y);}destroy(){this.sprite.destroy();this.name.destroy();}}

class Network extends Singleton{constructor(){super();}connect(url,game,instanced,callback){Network.instance.game=game;let socket=new WebSocket(url);Network.instance.socket=socket;socket.onopen=()=>{console.log("Connected to server");callback();};socket.onmessage=event=>{let{type,data}=JSON.parse(event.data);let player;let sprite;let uText;switch(type){case"login":if(data.success){console.log("Logged in");}else {console.log("Login failed");}break;case"selfJoin":console.log("Self joined",data.name);player=new Player(data.name,data.x,data.y);sprite=Network.instance.game.add.sprite(data.x,data.y,"idle1");uText=Network.instance.game.add.text(data.x,data.y,data.name,{font:"16px Arial",fill:"#ffffff",align:"center"});player.setSprite(sprite);player.setName(uText);player.animate("idle");instanced.addEntity(player);BaseGame.instance.camera.startFollow(player.sprite);break;case"playerJoin":if(data.name!==Network.instance.username){console.log("Player joined",data.name);player=new Player(data.name,data.x,data.y);sprite=Network.instance.game.add.sprite(data.x,data.y,"idle1");uText=Network.instance.game.add.text(data.x,data.y,data.name,{font:"16px Arial",fill:"#ffffff",align:"center"});player.setSprite(sprite);player.setName(uText);player.animate("idle");instanced.addEntity(player);}break;case"move":player=BaseGame.instance.findObject(data.name);if(player)player.move(data.x,data.y,data.direction);break;case"loadMap":instanced.loadMap(data.map);break;case"playerLeave":player=BaseGame.instance.findObject(data.name);if(player)player.destroy();break;case"setChange":BaseGame.instance.updates.set(data.x,data.y,data);break;case"deleteBlock":instanced.deleteBlock(data);break;case"chat":MainGame.instance.onMessage(data);break;case"worldMenu":MainGame.instance.onWorldMenu(data);break;default:console.log(`Unknown message type: ${type}`);break;}};socket.onclose=()=>{console.log("Disconnected from server");};}setActive(game){Network.instance.game=game;}send(data){this.socket.send(JSON.stringify(data));}}
...
InputSystem.instance.linkEvent("keydown",e=>{switch(e.key){case"w":Network.instance.send({type:"move",data:{x:0,y:-1}});break;case"a":Network.instance.send({type:"move",data:{x:-1,y:0}});break;case"s":Network.instance.send({type:"move",data:{x:0,y:1}});break;case"d":Network.instance.send({type:"move",data:{x:1,y:0}});break;}});InputSystem.instance.linkEvent("keyup",e=>{switch(e.key){case"w":Network.instance.send({type:"move",data:{x:0,y:0}});break;case"a":Network.instance.send({type:"move",data:{x:0,y:0}});break;case"s":Network.instance.send({type:"move",data:{x:0,y:0}});break;case"d":Network.instance.send({type:"move",data:{x:0,y:0}});break;}});this.input.on("pointerdown",pointer=>{// let { worldX, worldY } = pointer;
// worldX = Math.floor(worldX / 32) * 32;
// worldY = Math.floor(worldY / 32) * 32;
// console.log(worldX, worldY);
let instancedBlock=BaseGame.instance.instancedBlock;switch(pointer.button){// Left
case 0:Network.instance.send({type:"leftInteract",data:{name:instancedBlock.name,x:instancedBlock.x,y:instancedBlock.y}});break;// Right
case 2:Network.instance.send({type:"rightInteract",data:{name:instancedBlock.name,x:instancedBlock.x,y:instancedBlock.y}});break;}});}update(){if(!Loading.instance.checkBoot("game"))Loading.instance.setLoadingValues("game",this);let base=BaseGame.instance;for(let entity of base.objects.entities){}if(base.map&&!base.loadedMap){console.log(`Loading map with ${base.map.tiles.length} tiles`);for(let x=0;x<base.map.tiles.length;x++){let block=base.map.tiles[x];if(block){if(block.health<=0)return;block=block.value;let bl=this.add.sprite(block.x,block.y,block.name).setInteractive();bl.displayWidth=32;bl.displayHeight=32;if(BaseGame.instance.anims[`${block.name}`]){bl.play(`${block.name}_anim`,10,true);}bl.on("pointerover",()=>{BaseGame.instance.instancedBlock=block;});bl.setDepth(-1);if(block.health<3&&block.health>0){bl.cr=this.add.sprite(block.x,block.y,`cracked${3-block.health}`);}if(block.health<=0){bl.destroy();}else {BaseGame.instance.blocks.set(block.x,block.y,bl);}}}base.loadedMap=true;}for(let update of BaseGame.instance.updates.parts){update=update.value;let bl=BaseGame.instance.blocks.get(update.x,update.y);if(bl){bl=bl.value;if(bl.scene==undefined)return;bl.setTexture(update.name);bl.setDepth(-1);if(update.health<3){if(!bl.cr){bl.cr=this.add.sprite(update.x,update.y,`cracked${3-update.health}`);bl.cr.setDepth(0);}else {bl.cr.setTexture(`cracked${3-update.health}`);bl.cr.setDepth(0);}}else if(update.health==3){if(bl.cr){bl.cr.destroy();delete bl.cr;}}BaseGame.instance.updates.parts.splice(BaseGame.instance.updates.parts.indexOf(update),1);}}for(let block of BaseGame.instance.blocks.parts){block=block.value;let i=this.cameras.main.worldView.contains(block.x,block.y);let l=this.cameras.main.worldView.contains(block.x-32,block.y);let r=this.cameras.main.worldView.contains(block.x+32,block.y);let t=this.cameras.main.worldView.contains(block.x,block.y-32);let b=this.cameras.main.worldView.contains(block.x,block.y+32);if(i||l||r||t||b){block.visible=true;}else {block.visible=false;}}// let player = BaseGame.instance.findObject("player");
// if (player) {
//     player.move();
