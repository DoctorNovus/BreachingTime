...
class Singleton{setVariable(variable,value){this[variable]=value;}static get instance(){if(!this._instance){this._instance=new this();}return this._instance;}}

class Graphics extends Singleton{constructor(game){super();this.game=game;}rectangle(x,y,width,height,color){return this.game.add.rectangle(x,y,width,height,color);}}

class InputSystem extends Singleton{linkEvent(event,callback){this.game.input.keyboard.on(event,(...args)=>{if(document.activeElement==document.body)callback(...args);});}linkGenericEvent(event,callback){this.game.input.on(event,callback);}}

class Loading extends Singleton{setBoot(items){this.boot=items;}checkBoot(name){for(let i=0;i<this.boot.length;i++){if(!this.boot[i][name])return false;return true;}}setLoadingValues(name,value){for(let i=0;i<this.boot.length;i++){this.boot[i].instance.setVariable(name,value);}}}

class EventManager extends Singleton{constructor(){super();}setStart(game){this.game=game;Loading.instance.setBoot([Graphics,InputSystem]);EventManager.instance.blocks=[{key:"portal_sequence",type:"spritesheet",frameWidth:32,frameHeight:32},{key:"dirt",type:"single"},{key:"cinnabar",type:"single"},{key:"copper",type:"single"},{key:"gold",type:"single"},{key:"grass",type:"single"},{key:"iron",type:"single"},{key:"lead",type:"single"},{key:"nickel",type:"single"},{key:"silver",type:"single"},{key:"stone",type:"single"},{key:"zinc",type:"single"},{key:"cracked1",type:"single",overlay:true},{key:"cracked2",type:"single",overlay:true},{key:"cracked3",type:"single",overlay:true}];EventManager.instance.animations=[// Player Animations
{key:"idle",frames:["idle1","idle2","idle3","idle4","idle5","idle6","idle7","idle8","idle9","idle10"]},{key:"walking",frames:["walking1","walking2","walking3","walking4","walking5","walking6","walking7","walking8"]},{key:"jump",route:"jump",frames:["jump1","jump2","jump3","jump4","jump5","jump6","jump7"]},{key:"fall",route:"jump",frames:["jump8","jump9","jump10","jump11"]},{key:"jump_high",route:"jump",frames:["jump6"]},{key:"fall_low",route:"jump",frames:["jump10"]},// Tool Animations
{key:"fire_sword",route:"weapons/fire_sword",frames:["fire_sword1","fire_sword2","fire_sword3","fire_sword4","fire_sword5","fire_sword6","fire_sword7","fire_sword8","fire_sword9","fire_sword10"],origin:{x:0.5,y:0.5}}];}}

class GameConfig{constructor({type,parent,width,height,physics,scene}){let obj={};obj.type=type||Phaser.AUTO;if(parent)obj.parent=parent;if(!width&&!height){obj.width=window.innerWidth;obj.height=window.innerHeight;}else {obj.width=width;obj.height=height;}if(physics)obj.physics=physics;if(scene)obj.scene=scene;else {obj.scene={preload:EventManager.instance.preload,create:EventManager.instance.create,update:EventManager.instance.update};}return obj;}}

class Mapper$1{width=0;height=0;constructor(width,height=1){this.parts=[];this.width=width;this.height=height;if(width){for(let i=0;i<width;i++){for(let j=0;j<height;j++){this.set(i,j,0);}}}}loop(callback){for(let i=0;i<this.parts.length;i++){callback(this.parts[i]);}}set(x,y,value){if(!this.get(x,y))this.parts.push({x,y,value});else this.parts.find(part=>part.x===x&&part.y===y).value=value;return this.get(x,y);}get(x,y){let part=this.parts.find(part=>part.x==x&&part.y==y);if(part&&part.value)return part.value;else return null;}getFull(x,y){let part=this.parts.find(part=>part.x==x&&part.y==y);if(part)return part;else return null;}add(x,y,value){let part=this.get(x,y);if(!part)part=0;part+=value;this.set(x,y,part);return part;}divide(x,y,value){let part=this.get(x,y);if(!part)part=0;part/=value;this.set(x,y,part);return part;}asData(){let data=this.parts.map(part=>part.value.asData?part.value.asData():part.value);return data;}static from(mapper,parts){let w=0;let h=0;for(let i=0;i<parts.length;i++){let part=parts[i];let x=part.x;let y=part.y;if(w<x)w=x;if(h<y)h=y;mapper.set(x,y,part);}mapper.width=w+1;mapper.height=h+1;return mapper;}}

class BaseGame extends Singleton{constructor(config){super();// requestAnimationFrame(this.update.bind(this));
}start(config){this.config=config;this.game=new Phaser.Game(config);this.objects={entities:[],enemies:[],borders:[]};this.anims={};this.overlays=new Mapper$1();this.updates=new Mapper$1();this.blocks=new Mapper$1();}findObject(name){for(let obj in this.objects){for(let j=0;j<this.objects[obj].length;j++){let oj=this.objects[obj][j];if(oj.id==name)return oj;}}}addEntity(entity){this.objects.entities.push(entity);return entity;}addEnemy(enemy){this.objects.enemies.push(enemy);return enemy;}addBorder(border){this.objects.borders.push(border);return border;}checkCollides(obji){for(let obj in this.objects){for(let j=0;j<this.objects[obj].length;j++){let oj=this.objects[obj][j];if(oj!=obji&&oj.collides(obji))return true;}}return false;}setCamera(camera,zoom){this.camera=camera;this.camera.setZoom(zoom||1);}update(){EventManager.instance.update();requestAnimationFrame(this.update.bind(this));}loadMap(map){this.map=map;this.loadedMap=false;}deleteBlock(data){let block=this.blocks.get(data.x,data.y);if(block){if(block.value&&typeof block.value=="object")block=block.value;if(block.cr)block.cr.destroy();block.destroy();this.blocks.parts.splice(this.blocks.parts.indexOf(block),1);}}}

class Mapper{width=0;height=0;constructor(width,height=1){this.parts=[];this.width=width;this.height=height;if(width){for(let i=0;i<width;i++){for(let j=0;j<height;j++){this.set(i,j,0);}}}}loop(callback){for(let i=0;i<this.parts.length;i++){callback(this.parts[i]);}}set(x,y,value){if(!this.get(x,y))this.parts.push({x,y,value});else this.parts.find(part=>part.x===x&&part.y===y).value=value;return this.get(x,y);}get(x,y){let part=this.parts.find(part=>part.x==x&&part.y==y);if(part&&part.value)return part.value;else return null;}getFull(x,y){let part=this.parts.find(part=>part.x==x&&part.y==y);if(part)return part;else return null;}add(x,y,value){let part=this.get(x,y);if(!part)part=0;part+=value;this.set(x,y,part);return part;}divide(x,y,value){let part=this.get(x,y);if(!part)part=0;part/=value;this.set(x,y,part);return part;}asData(toggle){let parse=part=>{if(!toggle)return part.value;else return part;};let data=this.parts.map(part=>parse(part).asData?parse(part).asData():parse(part));return data;}static from(mapper,parts){let w=0;let h=0;for(let i=0;i<parts.length;i++){let part=parts[i];let x=part.x;let y=part.y;if(w<x)w=x;if(h<y)h=y;mapper.set(x,y,part);}mapper.width=w+1;mapper.height=h+1;return mapper;}}

class Zone{constructor(width,height,name){this.width=width;this.height=height;this.name=name;this.blocks=new Mapper();}addBlock(x,y,block){this.blocks.set(x,y,block);}getBlock(x,y){return this.blocks.get(x,y);}getBlockofValue(value){for(let block of this.blocks.asData()){if(block.value==value)return block;}return null;}deleteBlock(x,y){this.blocks.set(x,y,0);}asData(){return {width:this.width,height:this.height,name:this.name,spawnPoint:this.spawnPoint||{x:0,y:0},blocks:this.blocks.asData(true)};}static fromData(data){let zone=new Zone(data.width,data.height,data.name);zone.spawnPoint=data.spawnPoint;zone.blocks=Mapper.from(zone.blocks,data.blocks);return zone;}}

class FramesList extends Singleton{frames={// Player animations
"player_idle":[{texture:"idle1"},{texture:"idle2"},{texture:"idle3"},{texture:"idle4"},{texture:"idle5"},{texture:"idle6"},{texture:"idle7"},{texture:"idle8"},{texture:"idle9"},{texture:"idle10"}],"player_walking":[{texture:"walking1"},{texture:"walking2"},{texture:"walking3"},{texture:"walking4"},{texture:"walking5"},{texture:"walking6"},{texture:"walking7"},{texture:"walking8"}],"weapon_idle":[{origin:{x:0.5,y:0.5},angle:0}],// Weapon animations
"weapon_walking":[{origin:{x:0.3,y:0.75},angle:285},{origin:{x:0.35,y:0.70},angle:315},{origin:{x:0.35,y:0.5}},{origin:{x:0.5,y:0.5},angle:370},{origin:{x:0.5,y:0.45},angle:375},{origin:{x:0.4,y:0.40},angle:7},{origin:{x:0.4,y:0.6},angle:330},{origin:{x:0.3,y:0.7},angle:315}]};getFrames(name){return this.frames[name];}}

class Blueprint{constructor(name,type,frames,frameRate){this.name=name;this.type=type;this.frames=frames;this.frameRate=frameRate;}}

class BlueprintList extends Singleton{blueprints={// Player animations
"player_idle":new Blueprint("player_idle","movement",FramesList.instance.getFrames("player_idle"),10),"player_walking":new Blueprint("player_walking","movement",FramesList.instance.getFrames("player_walking"),10),// Weapon animations
"weapon_idle":new Blueprint("weapon_idle","weapon",FramesList.instance.getFrames("weapon_idle"),10),"weapon_walking":new Blueprint("weapon_walking","weapon",FramesList.instance.getFrames("weapon_walking"),10)};getBlueprint(name){return this.blueprints[name];}}
...
class Rectangle{/**
     ...
     * @param {number} width 
     * @param {number} height 
     */constructor(x,y,width,height){/** x location of the rectangle */this.x=x;/** y location of the rectangle */this.y=y;/** width of the rectangle */this.width=width;/** height of the rectangle */this.height=height;/** right side of the rectangle */this.right=this.x+this.width;/** bottom side of the rectangle */this.bottom=this.y+this.height;}/**
     ...
     */overlaps(rectangle){return this.x<rectangle.x+rectangle.width&&this.x+this.width>rectangle.x&&this.y<rectangle.y+rectangle.height&&this.y+this.height>rectangle.y;}/**
     * check if the rectangle is inside another rectangle
     * @param {Rectangle} rectangle rectangle to compare with
     * @return {boolean} is the rectangle inside the other rectangle
     */within(rectangle){return rectangle.x<=this.x&&rectangle.right>=this.right&&rectangle.y<=this.y&&rectangle.bottom>=this.bottom;}/**
...
class NPC{constructor(x,y,width,height){this.x=x;this.y=y;this.width=width;this.height=height;this.rect=new Rectangle();}collides(obj){if(this.rect.overlaps(obj.rect))return true;return false;}}

class Entity extends NPC{}

class CustomAnimations extends Singleton{blueprints=[];anims={};getBlueprint(name){return this.blueprints.find(blueprint=>blueprint.name===name);}stop(name){clearInterval(this.anims[name]);}stopAll(){for(let key in this.anims){clearInterval(this.anims[key]);}}renderBlueprint(blueprint,sprite,delay=0){setTimeout(()=>{let frameIndex=0;let anim=setInterval(()=>{if(frameIndex>=blueprint.frames.length&&blueprint.loop!=true){clearInterval(anim);return;}let frame=blueprint.frames[frameIndex];if(frame.origin)sprite.setOrigin(frame.origin.x,frame.origin.y);if(frame.rotation)sprite.rotation=frame.rotation;if(frame.scale)sprite.setScale(frame.scale.x,frame.scale.y);if(frame.texture)sprite.setTexture(frame.texture);if(frameIndex<blueprint.frames.length-1)frameIndex++;else frameIndex=0;},blueprint.frameRate||60);this.anims[blueprint.name]=anim;},delay);}renderOnUpdate(blueprint,sprite){if(!blueprint)return false;let core={index:0,name:blueprint.name,type:blueprint.type};if(this.anims[blueprint.type]&&this.anims[blueprint.type].name!=blueprint.name){this.anims[blueprint.type]=null;}else if(this.anims[blueprint.type]){core=this.anims[blueprint.type];}let timeCortex=this.anim(blueprint.frameRate,core,function render(){this.anims[blueprint.type].index++;// if (core.time > new Date() - blueprint.frameRate) {
if(core.index>=blueprint.frames.length)core.index=0;this.renderFrame(blueprint,sprite,core.index);core.time=Date.now();this.anims[blueprint.type]=core;});if(!timeCortex){// core.time = Date.now();
this.anims[blueprint.type]=core;}return true;}anim(fps,core,callback){let interv=1000/fps;let then=core.time||Date.now();let now=Date.now();let elapsed=now-then;if(elapsed>interv){then=now-elapsed%interv;callback.bind(this)();return true;}core.time=then;return false;}renderFrame(blueprint,sprite,frameCount){let frame=blueprint.frames[frameCount];if(frame.origin)sprite.setOrigin(frame.origin.x,frame.origin.y);if(frame.rotation){sprite.rotation=frame.rotation;}if(frame.angle){sprite.angle=frame.angle;}if(frame.scale)sprite.setScale(frame.scale.x,frame.scale.y);if(frame.texture)sprite.setTexture(frame.texture);}renderByName(name,sprite){let blueprint=this.getBlueprint(name);this.renderBlueprint(blueprint,sprite);}}

class Player extends Entity{constructor(id,x,y,width=32,height=32){super(x,y,width,height);this.id=id;this.velX=0;this.velY=0;}setSprite(sprite){this.sprite=sprite;this.sprite.x=this.x;this.sprite.y=this.y;this.sprite.width=this.width;this.sprite.height=this.height;}setName(name){this.name=name;this.name.x=this.x-this.width*.6;this.name.y=this.y-this.height;}setWeapon(weapon){this.weapon=weapon;this.weapon.x=this.x;this.weapon.y=this.y;this.weapon.sprite.x=this.weapon.x;this.weapon.sprite.y=this.weapon.y;this.weapon.sprite.setOrigin(this.weapon.origin.x,this.weapon.origin.y);}hideWeapon(){this.weapon.sprite.visible=false;}showWeapon(){this.weapon.sprite.visible=true;}canMove(){if(!BaseGame.instance.checkCollides(this))return true;return false;}move(x,y,direction="right"){this.sprite.x=x;this.sprite.y=y;this.name.x=x-this.width*.6;this.name.y=y-this.height;if(this.weapon){this.weapon.x=x;this.weapon.y=y;this.weapon.sprite.x=this.weapon.x;this.weapon.sprite.y=this.weapon.y;this.weapon.sprite.setOrigin(this.weapon.origin.x,this.weapon.origin.y);}if(direction=="left")this.animate("walking",-1);else if(direction=="right")this.animate("walking",1);else if(direction=="up"){if(this.currentAnim!="up"){this.animate("jump",1);}else {this.animate("jump_high",1);}}else if(direction=="down"){if(this.currentAnim!="down"){this.animate("fall",1);}else {this.animate("fall_low",1);}}else {this.animate("idle",1,true);}this.currentAnim=direction;}animate(animation,direction,loop){this.sprite.setScale(direction,1);if(this.weapon)this.weapon.sprite.setScale(direction,1);let pp=BlueprintList.instance.getBlueprint(`player_${animation}`);let wp=BlueprintList.instance.getBlueprint(`weapon_${animation}`);if(this.looped)clearInterval(this.looped);if(!loop){CustomAnimations.instance.renderOnUpdate(pp,this.sprite);if(this.weapon){CustomAnimations.instance.renderOnUpdate(wp,this.weapon.sprite);}}else {this.looped=setInterval(()=>{CustomAnimations.instance.renderOnUpdate(pp,this.sprite);if(this.weapon){this.weapon.sprite.angle=0;CustomAnimations.instance.renderOnUpdate(wp,this.weapon.sprite);}});}}destroy(){this.sprite.destroy();this.name.destroy();if(this.weapon)this.weapon.sprite.destroy();}}

class Network extends Singleton{constructor(){super();}connect(url,game,instanced,callback){Network.instance.game=game;let socket=new WebSocket(url);Network.instance.socket=socket;socket.onopen=()=>{console.log("Connected to server");callback();};socket.onmessage=event=>{let{type,data}=JSON.parse(event.data);let player;let sprite;let uText;let weapon;switch(type){case"login":if(data.success){console.log("Logged in");}else {console.log("Login failed");}break;case"selfJoin":console.log("Self joined",data.name);player=new Player(data.name,data.x,data.y);window.globalPlayerEntity=player;sprite=Network.instance.game.add.sprite(data.x,data.y,"idle1");weapon={sprite:Network.instance.game.add.sprite(data.x,data.y,"fire_sword1"),animation:"fire_sword",origin:{x:0.5,y:0.5}};uText=Network.instance.game.add.text(data.x,data.y,data.name,{font:"16px Arial",fill:"#ffffff",align:"center"});player.setSprite(sprite);player.setName(uText);player.setWeapon(weapon);player.animate("idle",1,true);instanced.addEntity(player);BaseGame.instance.camera.startFollow(player.sprite);break;case"playerJoin":if(data.name!==Network.instance.username){console.log("Player joined",data.name);player=new Player(data.name,data.x,data.y);sprite=Network.instance.game.add.sprite(data.x,data.y,"idle1");weapon={sprite:Network.instance.game.add.sprite(data.x,data.y,"fire_sword1"),animation:"fire_sword",origin:{x:0.5,y:0.5}};uText=Network.instance.game.add.text(data.x,data.y,data.name,{font:"16px Arial",fill:"#ffffff",align:"center"});player.setSprite(sprite);player.setName(uText);player.setWeapon(weapon);player.animate("idle",1,true);instanced.addEntity(player);}break;case"move":player=BaseGame.instance.findObject(data.name);if(player)player.move(data.x,data.y,data.direction);break;case"loadMap":instanced.loadMap(data.map);break;case"playerLeave":player=BaseGame.instance.findObject(data.name);if(player)player.destroy();break;case"setChange":BaseGame.instance.updates.set(data.x,data.y,data);break;case"deleteBlock":instanced.deleteBlock(data);break;case"chat":MainGame.instance.onMessage(data);break;case"worldMenu":MainGame.instance.onWorldMenu(data);break;case"inventoryUpdate":MainGame.instance.onInventory(data);break;default:console.log(`Unknown message type: ${type}`);break;}};socket.onclose=()=>{console.log("Disconnected from server");};}setActive(game){Network.instance.game=game;}send(data){if(data.asData){if(data instanceof Zone){let zData=data.asData();let zDataBlocks=[];if(zData&&zData.blocks[0].value.zone)zData.blocks.forEach(block=>{let b=block.value;zDataBlocks.push({x:b.x,y:b.y,width:b.width,height:b.height,value:b.value});});zData.blocks=zDataBlocks;this.socket.send(JSON.stringify(zData));}else {this.socket.send(JSON.stringify(data));}}else {this.socket.send(JSON.stringify(data));}}}

class StartScene extends Phaser.Scene{constructor(){super("StartScene");}preload(){console.log("STARTING SCENE");}create(){let that=this;BaseGame.instance.network=new Network();BaseGame.instance.network.connect("ws://localhost:8080",this,BaseGame.instance,()=>{that.scene.start("GameScene");});}update(){}}

class BlockIndex{static _blocks={0:"air",1:"portal_sequence",2:"grass",3:"dirt",4:"stone",5:"cinnabar",6:"copper",7:"gold",8:"iron",9:"lead",10:"lead",11:"nickel",12:"silver",13:"zinc"};static get blocks(){return BlockIndex._blocks;}}

class GameScene extends Phaser.Scene{constructor(){super("GameScene");}preload(){if(!Loading.instance.checkBoot("game"))Loading.instance.setLoadingValues("game",this);for(let i=0;i<EventManager.instance.animations.length;i++){let anim=EventManager.instance.animations[i];for(let j=0;j<anim.frames.length;j++){this.load.image(anim.frames[j],`assets/animations/${anim.route?anim.route:anim.key}/${anim.frames[j]}.png`);}}for(let i=0;i<EventManager.instance.blocks.length;i++){let block=EventManager.instance.blocks[i];let area=block.overlay?"overlays":"blocks";if(block.type==="single"){this.load.image(block.key,`assets/${area}/${block.key}.png`);}else if(block.type==="spritesheet"){this.load.spritesheet(block.key,`assets/${area}/${block.key}.png`,{frameWidth:block.frameWidth,frameHeight:block.frameHeight});}else {for(let j=0;j<block.frames.length;j++){this.load.image(block.frames[j],`assets/${area}/${block.key}/${block.frames[j]}.png`);}}}}create(){if(!Loading.instance.checkBoot("game"))Loading.instance.setLoadingValues("game",this);for(let i=0;i<EventManager.instance.animations.length;i++){let anim=EventManager.instance.animations[i];this.anims.create({key:anim.key,frames:anim.frames.map(an=>({key:an})),frameRate:10,repeat:anim.repeat?anim.repeat:-1});}for(let i=0;i<EventManager.instance.blocks.length;i++){let block=EventManager.instance.blocks[i];if(block.type==="spritesheet"){let frames=this.anims.generateFrameNames(block.key);BaseGame.instance.anims[block.key]=this.anims.create({key:`${block.key}_anim`,frames,frameRate:10,repeat:-1});}else if(block.type=="multi"){BaseGame.instance.anims[block.key]=this.anims.create({key:`${block.key}_anim`,frames:block.frames.map(an=>({key:an})),frameRate:10,repeat:-1});}}BaseGame.instance.setCamera(this.cameras.main,1.5);let username=BaseGame.instance.username;console.log(`Username: ${username}`);Network.instance.username=username;Network.instance.send({type:"login",data:{name:username}});Network.instance.setActive(this);InputSystem.instance.linkEvent("keydown",e=>{let newZoom;switch(e.key){case"w":Network.instance.send({type:"move",data:{x:0,y:-1}});break;case"a":Network.instance.send({type:"move",data:{x:-1,y:0}});break;case"s":Network.instance.send({type:"move",data:{x:0,y:1}});break;case"d":Network.instance.send({type:"move",data:{x:1,y:0}});break;case"-":newZoom=this.cameras.main.zoom-.1;if(newZoom>0.6){this.cameras.main.zoom=newZoom;}break;case"=":newZoom=this.cameras.main.zoom+.1;if(newZoom<3){this.cameras.main.zoom=newZoom;}break;default:MainGame.instance.onKeybind(e.key);break;}});InputSystem.instance.linkEvent("keyup",e=>{switch(e.key){case"w":Network.instance.send({type:"move",data:{x:0,y:0}});break;case"a":Network.instance.send({type:"move",data:{x:0,y:0}});break;case"s":Network.instance.send({type:"move",data:{x:0,y:0}});break;case"d":Network.instance.send({type:"move",data:{x:0,y:0}});break;}});}update(){if(!Loading.instance.checkBoot("game"))Loading.instance.setLoadingValues("game",this);let base=BaseGame.instance;for(let entity of base.objects.entities){}if(base.map&&!base.loadedMap){let blockMap=new Mapper$1();blockMap=Mapper$1.from(blockMap,base.map);for(let x=0;x<blockMap.parts.length;x++){let block=blockMap.parts[x];if(block){if(typeof block.health!="undefined"&&block.health<=0)return;block={x:block.x,y:block.y,value:block.value.value};let blockName=BlockIndex.blocks[block.value];let bl=this.add.sprite(block.x*32,block.y*32,blockName).setInteractive();bl.displayWidth=32;bl.displayHeight=32;if(BaseGame.instance.anims[`${blockName}`]){bl.play(`${blockName}_anim`,10,true);}bl.on("pointerdown",pointer=>{switch(pointer.button){// Left
case 0:Network.instance.send({type:"leftInteract",data:{name:blockName,x:block.x,y:block.y}});break;// Right
case 2:Network.instance.send({type:"rightInteract",data:{name:blockName,x:block.x,y:block.y}});break;}});bl.setDepth(-1);if(typeof block.health!="undefined"&&block.health<3&&block.health>0){bl.cr=this.add.sprite(block.x,block.y,`cracked${3-block.health}`);}if(typeof block.health!="undefined"&&block.health<=0){bl.destroy();}else {BaseGame.instance.blocks.set(block.x,block.y,bl);}}}base.loadedMap=true;}for(let update of BaseGame.instance.updates.parts){update=update.value;let bl=BaseGame.instance.blocks.get(update.x,update.y);if(bl){bl.setTexture(BlockIndex.blocks[update.value]);bl.setDepth(-1);if(update.health<3){if(!bl.cr){bl.cr=this.add.sprite(update.x*32,update.y*32,`cracked${3-update.health}`);bl.cr.displayWidth=32;bl.cr.displayHeight=32;bl.cr.setDepth(0);}else {bl.cr.setTexture(`cracked${3-update.health}`);bl.cr.displayWidth=32;bl.cr.displayHeight=32;bl.cr.setDepth(0);}}else if(update.health==3){if(bl.cr){bl.cr.destroy();delete bl.cr;}}BaseGame.instance.blocks.set(update.x,update.y,bl);BaseGame.instance.updates.parts.splice(BaseGame.instance.updates.parts.indexOf(update),1);}}}}

class MainGame extends Singleton{start(username){BaseGame.instance.username=username;let config=new GameConfig({parent:"game",scene:[StartScene,GameScene]});BaseGame.instance.start(config);let em=new EventManager();em.setStart(game);}onMessage(data){console.log(data);}onWorldMenu(data){console.log(data);}}

function WorldMenu({worlds,shown,setSelected,mode}){let[area,setArea]=React.useState(0);return/*#__PURE__*/React.createElement("div",{className:`world-menu-main ${shown?"":"hidden"}`},/*#__PURE__*/React.createElement("div",{className:"world-menu-side"},/*#__PURE__*/React.createElement("div",{className:"world-menu-sub",onClick:()=>setArea(0)},/*#__PURE__*/React.createElement("div",{className:"world-menu-title"},/*#__PURE__*/React.createElement("img",{src:"/assets/ui/panel0.png",alt:"bg"}),/*#__PURE__*/React.createElement("p",null,"Explore"))),/*#__PURE__*/React.createElement("div",{className:"world-menu-sub",onClick:()=>setArea(1)},/*#__PURE__*/React.createElement("div",{className:"world-menu-title"},/*#__PURE__*/React.createElement("img",{src:"/assets/ui/panel0.png",alt:"bg"}),/*#__PURE__*/React.createElement("p",null,"Personal"))),/*#__PURE__*/React.createElement("div",{className:"world-menu-sub",onClick:()=>setArea(2)},/*#__PURE__*/React.createElement("div",{className:"world-menu-title"},/*#__PURE__*/React.createElement("img",{src:"/assets/ui/panel0.png",alt:"bg"}),/*#__PURE__*/React.createElement("p",null,"Search")))),/*#__PURE__*/React.createElement("div",{className:"world-menu-container"},/*#__PURE__*/React.createElement("div",{className:`world-menu-explore ${!mode||mode=="explore"?"":"hidden"}`},/*#__PURE__*/React.createElement("div",null,/*#__PURE__*/React.createElement("div",{className:`world-menu-worlds ${area!=0?"hidden":""}`},/*#__PURE__*/React.createElement("div",{className:"fd"},/*#__PURE__*/React.createElement("select",{className:"world-menu-sort",onChange:e=>selectFilter(e)},/*#__PURE__*/React.createElement("option",null,"PVP"),/*#__PURE__*/React.createElement("option",null,"Popular"),/*#__PURE__*/React.createElement("option",null,"Unexplored"),/*#__PURE__*/React.createElement("option",null,"Developed"),/*#__PURE__*/React.createElement("option",null,"Trade"))),/*#__PURE__*/React.createElement("div",{className:"sd"},/*#__PURE__*/React.createElement("ul",{className:"world-menu-worldList",id:"worldList"},worlds.map((world,i)=>{return/*#__PURE__*/React.createElement("li",{key:i,onClick:()=>{setSelected(world.name);Network.instance.send({type:"worldSelect",data:{name:world.name}});}},/*#__PURE__*/React.createElement("div",{className:"world-menu-world"},/*#__PURE__*/React.createElement("div",{className:"world-menu-world-name"},/*#__PURE__*/React.createElement("p",null,world.name)),/*#__PURE__*/React.createElement("div",{className:"world-menu-world-info"},/*#__PURE__*/React.createElement("p",null,world.players,"/",world.maxPlayers))));})))),/*#__PURE__*/React.createElement("div",{className:`world-menu-personal ${area!=1?"hidden":""}`}),/*#__PURE__*/React.createElement("div",{className:`world-menu-search ${area!=2?"hidden":""}`},/*#__PURE__*/React.createElement("form",{onSubmit:e=>{e.preventDefault();searchWorld(worlds,setSelected);}},/*#__PURE__*/React.createElement("div",{className:"world-menu-search-input"},/*#__PURE__*/React.createElement("input",{name:"search",id:"searchWorld",type:"text",placeholder:"World Name..."})),/*#__PURE__*/React.createElement("div",{className:"world-menu-search-button"},/*#__PURE__*/React.createElement("img",{src:"/assets/ui/panel1.png",alt:"bg"}),/*#__PURE__*/React.createElement("button",{type:"submit"},"Search"))),/*#__PURE__*/React.createElement("div",{className:"world-menu-search-results"},/*#__PURE__*/React.createElement("ul",{className:"world-menu-search-results-list"},worlds.map((world,i)=>{return/*#__PURE__*/React.createElement("li",{key:i,onClick:()=>{setSelected(world.name);Network.instance.send({type:"worldSelect",data:{name:world.name}});}},/*#__PURE__*/React.createElement("div",{className:"world-menu-world"},/*#__PURE__*/React.createElement("div",{className:"world-menu-world-name"},/*#__PURE__*/React.createElement("p",null,world.name))));}))))))));}function selectFilter(e){let filter=e.target.value;console.log(filter);}function searchWorld(worlds,setSelected){let world=document.getElementById("searchWorld").value;let selected=worlds.find(w=>w.name==world);if(selected){setSelected(selected.name);Network.instance.send({type:"worldSelect",data:{name:selected.name}});}else {setSelected(world);Network.instance.send({type:"worldCreate",data:{name:world}});}}

function CharacterBuild({profile}){if(!profile){return/*#__PURE__*/React.createElement(React.Fragment,null);}return/*#__PURE__*/React.createElement(React.Fragment,null);}

function InventoryItem({item,active,setActive,slot}){let iName="";if(!item){item={};}else if(item.id!=0){console.log(item);iName=BlockIndex.blocks[item.id];console.log(iName);}if(slot)return/*#__PURE__*/React.createElement("div",{className:"inventory-slot",id:`slot${slot}`},/*#__PURE__*/React.createElement("div",{className:"inventory-item-image"},iName&&iName.trim().length>0?/*#__PURE__*/React.createElement("img",{src:`assets/blocks/${iName}.png`,alt:"slotItem"}):/*#__PURE__*/React.createElement("div",{className:"inventory-item-image-empty"})),/*#__PURE__*/React.createElement("div",{className:"inventory-item-count"},item.count&&item.count>0?/*#__PURE__*/React.createElement("span",null,item.count):/*#__PURE__*/React.createElement("span",null)));else return/*#__PURE__*/React.createElement("div",{className:`inventory-item ${active.id==(item.id||0)?"glow":""}`,onClick:()=>setActive(item)},/*#__PURE__*/React.createElement("div",{className:"inventory-item-image"},iName&&iName.trim().length>0?/*#__PURE__*/React.createElement("img",{src:`assets/blocks/${iName}.png`,alt:"slotItem"}):/*#__PURE__*/React.createElement("div",{className:"inventory-item-image-empty"})),/*#__PURE__*/React.createElement("div",{className:"inventory-item-count"},item.count&&item.count>0?/*#__PURE__*/React.createElement("span",null,item.count):/*#__PURE__*/React.createElement("span",null)));}

function Inventory({inventory,profile,shown}){if(!inventory)inventory=new Array(64).fill({id:0,count:0,img:""});else if(inventory.length<64){inventory=inventory.concat(new Array(64-inventory.length).fill({id:0,count:0,img:""}));}if(!profile)profile={};let[active,setActive]=react.exports.useState({});return/*#__PURE__*/React.createElement("div",{className:`inventory ${shown?"":"hidden"}`},/*#__PURE__*/React.createElement("div",{className:"inventory-profile"},/*#__PURE__*/React.createElement("div",{className:"inventory-profile-name"},/*#__PURE__*/React.createElement("span",null,profile.name||"No Name"),/*#__PURE__*/React.createElement("span",null,"Lvl. ",profile.level||1)),/*#__PURE__*/React.createElement("div",{className:"inventory-profile-build"},/*#__PURE__*/React.createElement("div",{className:"inventory-profile-top"},/*#__PURE__*/React.createElement(InventoryItem,{item:profile.slots&&profile.slots.leftHand,slot:1}),/*#__PURE__*/React.createElement(InventoryItem,{item:profile.slots&&profile.slots.head,slot:2}),/*#__PURE__*/React.createElement(InventoryItem,{item:profile.slots&&profile.slots.backPiece,slot:3}),/*#__PURE__*/React.createElement(InventoryItem,{item:profile.slots&&profile.slots.legs,slot:4})),/*#__PURE__*/React.createElement("div",{className:"inventory-profile-center"},/*#__PURE__*/React.createElement("div",{className:"inventory-profile-center-container"},/*#__PURE__*/React.createElement(CharacterBuild,{profile:profile}))),/*#__PURE__*/React.createElement("div",{className:"inventory-profile-bottom"},/*#__PURE__*/React.createElement(InventoryItem,{item:profile.slots&&profile.slots.rightHand,slot:5}),/*#__PURE__*/React.createElement(InventoryItem,{item:profile.slots&&profile.slots.chest,slot:6}),/*#__PURE__*/React.createElement(InventoryItem,{item:profile.slots&&profile.slots.feet,slot:7})))),/*#__PURE__*/React.createElement("div",{className:"inventory-items"},/*#__PURE__*/React.createElement("ul",null,inventory.map((item,index)=>{return/*#__PURE__*/React.createElement("li",{key:index},/*#__PURE__*/React.createElement(InventoryItem,{item:item,active:active,setActive:setActive}));}))));}

const chatbox=/*#__PURE__*/React.createRef();const status=/*#__PURE__*/React.createRef();function App(){const[loginMode,setLoginMode]=React.useState(true);const[loggedIn,setLoggedIn]=React.useState(false);const[started,setStarted]=React.useState(false);const[username,setUsername]=React.useState("");const[selected,setSelected]=React.useState("");const[worlds,setWorlds]=React.useState([]);const[character,setCharacter]=React.useState({});const[kb,setKb]=React.useState({kb:[]});return/*#__PURE__*/React.createElement("div",{className:"main"},/*#__PURE__*/React.createElement("div",{className:`main-first ${started?"hidden":""}`},/*#__PURE__*/React.createElement("div",{className:`main-first-login ${loggedIn?"hidden":""}`},/*#__PURE__*/React.createElement("div",{className:`main-first-login-body`},/*#__PURE__*/React.createElement("h1",null,`${loginMode?"Login":"Register"}`),/*#__PURE__*/React.createElement("form",{onSubmit:e=>online.bind(this)(e,loginMode,setLoggedIn,setUsername)},/*#__PURE__*/React.createElement("label",{ref:status}),/*#__PURE__*/React.createElement("label",{htmlFor:"uname"},"Username"),/*#__PURE__*/React.createElement("input",{autoComplete:"false",className:"uname",type:"text",name:"uname"}),/*#__PURE__*/React.createElement("div",{className:`main-first-login-body-email ${loginMode?"hidden":""}`},/*#__PURE__*/React.createElement("label",{htmlFor:"email"},"Email"),/*#__PURE__*/React.createElement("input",{autoComplete:"false",className:"email",type:"email",name:"email"})),/*#__PURE__*/React.createElement("label",{htmlFor:"pass"},"Password"),/*#__PURE__*/React.createElement("input",{autoComplete:"false",className:"pass",type:"password",name:"pass"}),/*#__PURE__*/React.createElement("input",{className:"submitEntry",type:"submit",value:`${loginMode?"Login":"Register"}`}),/*#__PURE__*/React.createElement("span",{onClick:()=>{setLoginMode(!loginMode);}},"Switch to ",`${loginMode?"Register":"Login"}`,"?")))),/*#__PURE__*/React.createElement("div",{className:`main-first-game ${loggedIn?"":"hidden"}`,onClick:()=>{startGame.bind(this)(setStarted,username,setWorlds,setCharacter,kb,setKb);}},/*#__PURE__*/React.createElement("img",{src:"/assets/ui/startBG.gif",alt:"startBG"}),/*#__PURE__*/React.createElement("em",{id:"startButton"},"Click the screen to start..."))),/*#__PURE__*/React.createElement("div",{className:`main-second ${started?"":"hidden"}`},/*#__PURE__*/React.createElement(WorldMenu,{worlds:worlds,setSelected:setSelected,shown:selected.length==0?true:false}),/*#__PURE__*/React.createElement("div",{className:`${selected.length==0?"hidden":""}`},/*#__PURE__*/React.createElement(Inventory,{inventory:character.inventory,profile:character.profile,shown:kb.kb.includes("i")}))));}async function online(e,loginMode,setLoggedIn,setUsername){e.preventDefault();if(loginMode){await login.bind(this)(e,setLoggedIn,setUsername);}else {await register.bind(this)(e,setLoggedIn,setUsername);}}async function login(e,setLoggedIn,setUsername){e.preventDefault();let uname=document.querySelector("input[name=uname]");let pass=document.querySelector("input[name=pass]");let res=await fetch("/api/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({uname:uname.value,pass:pass.value})});res=await res.json();if(res.success){console.log(res.user.name);setLoggedIn(true);setUsername(res.user.name);}else {status.current.innerHTML=res.message;}}async function register(e,setLoggedIn,setUsername){e.preventDefault();let uname=document.querySelector("input[name=uname]");let email=document.querySelector("input[name=email]");let pass=document.querySelector("input[name=pass]");let res=await fetch("/api/register",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({uname:uname.value,email:email.value,pass:pass.value})});res=await res.json();if(res.success){setLoggedIn(true);setUsername(res.user.name);}else {status.current.innerHTML=res.message;}}function startGame(setStarted,username,setWorlds,setCharacter,keybinds,setKeybindElems){setStarted(true);let game=new MainGame();MainGame.instance.onMessage=data=>{chatbox.current.chat(data);};MainGame.instance.onWorldMenu=data=>{let{worlds}=data;setWorlds(worlds);};MainGame.instance.onInventory=data=>{let{items,profile}=data;setCharacter({inventory:items,profile});};MainGame.instance.onKeybind=data=>{let kb=keybinds.kb;if(!kb.includes(data)){kb.push(data);}else {kb.splice(kb.indexOf(data),1);}setKeybindElems({kb});};game.start(username);}

ReactDOM.render(/*#__PURE__*/React.createElement(App,null),document.getElementById("ui"));
