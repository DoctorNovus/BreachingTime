...
class Player extends Entity{constructor(id,x,y,width=32,height=32){super(x,y,width,height);this.id=id;this.velX=0;this.velY=0;}setSprite(sprite){this.sprite=sprite;this.sprite.x=this.x;this.sprite.y=this.y;this.sprite.width=this.width;this.sprite.height=this.height;}setName(name){this.name=name;this.name.x=this.x-this.width*.6;this.name.y=this.y-this.height;}canMove(){if(!BaseGame.instance.checkCollides(this))return true;return false;}move(x,y,direction="right"){this.sprite.x=x;this.sprite.y=y;this.name.x=x-this.width*.6;this.name.y=y-this.height;if(direction=="left")this.animate("walking",-1);else if(direction=="right")this.animate("walking",1);else this.animate("idle",1);}animate(animation,direction){this.sprite.play(animation,10,true);this.sprite.setScale(direction,1);}destroy(){this.sprite.destroy();}}

class Network extends Singleton{constructor(){super();}connect(url,game,instanced,callback){Network.instance.game=game;let socket=new WebSocket(url);Network.instance.socket=socket;socket.onopen=()=>{console.log("Connected to server");callback();};socket.onmessage=event=>{let{type,data}=JSON.parse(event.data);let player;let sprite;let uText;switch(type){case"login":if(data.success){console.log("Logged in");}else {console.log("Login failed");}break;case"selfJoin":console.log("Self joined",data.name);player=new Player(data.name,data.x,data.y);sprite=Network.instance.game.add.sprite(data.x,data.y,"idle1");uText=Network.instance.game.add.text(data.x,data.y,data.name,{font:"16px Arial",fill:"#ffffff",align:"center"});player.setSprite(sprite);player.setName(uText);player.animate("idle");instanced.addEntity(player);BaseGame.instance.camera.startFollow(player.sprite);break;case"playerJoin":if(data.name!==Network.instance.username){console.log("Player joined",data.name);player=new Player(data.name,data.x,data.y);sprite=Network.instance.game.add.sprite(data.x,data.y,"idle1");uText=Network.instance.game.add.text(data.x,data.y,data.name,{font:"16px Arial",fill:"#ffffff",align:"center"});player.setSprite(sprite);player.setName(uText);player.animate("idle");instanced.addEntity(player);}break;case"move":player=BaseGame.instance.findObject(data.name);if(player)player.move(data.x,data.y,data.direction);break;case"loadMap":instanced.loadMap(data.map);break;case"playerLeave":player=BaseGame.instance.findObject(data.name);if(player)player.destroy();break;case"setChange":BaseGame.instance.updates.set(data.x,data.y,data);break;case"deleteBlock":instanced.deleteBlock(data);break;case"chat":MainGame.instance.onMessage(data);break;default:console.log(`Unknown message type: ${type}`);break;}};socket.onclose=()=>{console.log("Disconnected from server");};}setActive(game){Network.instance.game=game;}send(data){this.socket.send(JSON.stringify(data));}}
...
class GameScene extends Phaser.Scene{constructor(){super("GameScene");}preload(){console.log("TEST");if(!Loading.instance.checkBoot("game"))Loading.instance.setLoadingValues("game",this);for(let i=0;i<EventManager.instance.animations.length;i++){let anim=EventManager.instance.animations[i];for(let j=0;j<anim.frames.length;j++){this.load.image(anim.frames[j],`assets/animations/${anim.key}/${anim.frames[j]}.png`);}}for(let i=0;i<EventManager.instance.blocks.length;i++){let block=EventManager.instance.blocks[i];let area=block.overlay?"overlays":"blocks";if(block.type==="single"){this.load.image(block.key,`assets/${area}/${block.key}.png`);}else if(block.type==="spritesheet"){this.load.spritesheet(block.key,`assets/${area}/${block.key}.png`,{frameWidth:block.frameWidth,frameHeight:block.frameHeight});}else {for(let j=0;j<block.frames.length;j++){this.load.image(block.frames[j],`assets/${area}/${block.key}/${block.frames[j]}.png`);}}}}create(){if(!Loading.instance.checkBoot("game"))Loading.instance.setLoadingValues("game",this);for(let i=0;i<EventManager.instance.animations.length;i++){let anim=EventManager.instance.animations[i];this.anims.create({key:anim.key,frames:anim.frames.map(an=>({key:an})),frameRate:10,repeat:-1});}for(let i=0;i<EventManager.instance.blocks.length;i++){let block=EventManager.instance.blocks[i];if(block.type==="spritesheet"){let frames=this.anims.generateFrameNames(block.key);BaseGame.instance.anims[block.key]=this.anims.create({key:`${block.key}_anim`,frames,frameRate:10,repeat:-1});}else if(block.type=="multi"){BaseGame.instance.anims[block.key]=this.anims.create({key:`${block.key}_anim`,frames:block.frames.map(an=>({key:an})),frameRate:10,repeat:-1});}}BaseGame.instance.setCamera(this.cameras.main,1.5);let username=BaseGame.instance.username;console.log(`Username: ${username}`);Network.instance.username=username;Network.instance.send({type:"login",data:{name:username}});Network.instance.setActive(this);// let player = new Player(100, 100, 32, 32, this.add.sprite("idle1"), "player");
