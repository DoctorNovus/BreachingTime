...
class InputSystem extends Singleton{linkEvent(event,callback){this.game.input.keyboard.on(event,callback);}linkGenericEvent(event,callback){this.game.input.on(event,callback);}}
...
class EventManager extends Singleton{constructor(){super();}setStart(game){this.game=game;Loading.instance.setBoot([Graphics,InputSystem]);EventManager.instance.blocks=[{key:"dirt",type:"single"},{key:"portal_sequence",type:"spritesheet",frameWidth:32,frameHeight:32},{key:"cracked1",type:"single",overlay:true},{key:"cracked2",type:"single",overlay:true},{key:"cracked3",type:"single",overlay:true}];EventManager.instance.animations=[// Player Animations
...
class Mapper{constructor(){this.parts=[];}loop(callback){for(let i=0;i<this.parts.length;i++){callback(this.parts[i]);}}set(x,y,value){if(!this.get(x,y))this.parts.push({x,y,value});else this.parts.find(part=>part.x===x&&part.y===y).value=value;return this.get(x,y);}get(x,y){let part=this.parts.find(part=>part.x==x&&part.y==y);if(part)return part;else return null;}add(x,y,value){let part=this.get(x,y);if(!part)part=0;part+=value;this.set(x,y,part);return part;}divide(x,y,value){let part=this.get(x,y);if(!part)part=0;part/=value;this.set(x,y,part);return part;}}
...
InputSystem.instance.linkEvent("keydown",e=>{let newZoom;switch(e.key){case"w":Network.instance.send({type:"move",data:{x:0,y:-1}});break;case"a":Network.instance.send({type:"move",data:{x:-1,y:0}});break;case"s":Network.instance.send({type:"move",data:{x:0,y:1}});break;case"d":Network.instance.send({type:"move",data:{x:1,y:0}});break;case"-":newZoom=this.cameras.main.zoom-.1;console.log(newZoom);if(newZoom>0.6){this.cameras.main.zoom=newZoom;}break;case"=":newZoom=this.cameras.main.zoom+.1;console.log(newZoom);if(newZoom<3){this.cameras.main.zoom=newZoom;}break;default:console.log(`Unknown key: ${e.key}`);break;}});InputSystem.instance.linkEvent("keyup",e=>{switch(e.key){case"w":Network.instance.send({type:"move",data:{x:0,y:0}});break;case"a":Network.instance.send({type:"move",data:{x:0,y:0}});break;case"s":Network.instance.send({type:"move",data:{x:0,y:0}});break;case"d":Network.instance.send({type:"move",data:{x:0,y:0}});break;}});}update(){if(!Loading.instance.checkBoot("game"))Loading.instance.setLoadingValues("game",this);let base=BaseGame.instance;for(let entity of base.objects.entities){}if(base.map&&!base.loadedMap){console.log(`Loading map with ${base.map.tiles.length} tiles`);for(let x=0;x<base.map.tiles.length;x++){let block=base.map.tiles[x];if(block){if(block.health<=0)return;block=block.value;let bl=this.add.sprite(block.x,block.y,block.name).setInteractive();bl.displayWidth=32;bl.displayHeight=32;if(BaseGame.instance.anims[`${block.name}`]){bl.play(`${block.name}_anim`,10,true);}bl.on("pointerdown",pointer=>{switch(pointer.button){// Left
case 0:Network.instance.send({type:"leftInteract",data:{name:block.name,x:block.x,y:block.y}});break;// Right
case 2:Network.instance.send({type:"rightInteract",data:{name:block.name,x:block.x,y:block.y}});break;}});bl.setDepth(-1);if(block.health<3&&block.health>0){bl.cr=this.add.sprite(block.x,block.y,`cracked${3-block.health}`);}if(block.health<=0){bl.destroy();}else {BaseGame.instance.blocks.set(block.x,block.y,bl);}}}base.loadedMap=true;}for(let update of BaseGame.instance.updates.parts){update=update.value;let bl=BaseGame.instance.blocks.get(update.x,update.y);if(bl){bl=bl.value;if(bl.scene==undefined)return;bl.setTexture(update.name);bl.setDepth(-1);if(update.health<3){if(!bl.cr){bl.cr=this.add.sprite(update.x,update.y,`cracked${3-update.health}`);bl.cr.setDepth(0);}else {bl.cr.setTexture(`cracked${3-update.health}`);bl.cr.setDepth(0);}}else if(update.health==3){if(bl.cr){bl.cr.destroy();delete bl.cr;}}BaseGame.instance.updates.parts.splice(BaseGame.instance.updates.parts.indexOf(update),1);}}for(let block of BaseGame.instance.blocks.parts){block=block.value;let i=this.cameras.main.worldView.contains(block.x,block.y);let l=this.cameras.main.worldView.contains(block.x-32,block.y);let r=this.cameras.main.worldView.contains(block.x+32,block.y);let t=this.cameras.main.worldView.contains(block.x,block.y-32);let b=this.cameras.main.worldView.contains(block.x,block.y+32);if(i||l||r||t||b){block.visible=true;}else {block.visible=false;}}// let player = BaseGame.instance.findObject("player");
// if (player) {
//     player.move();
